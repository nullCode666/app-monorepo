...
import { WALLET_SELECTOR_TOTAL, WALLET_SELECTOR_WALLETS, type WalletSelectorWallet } from '@/core/constants/wallet';
...
export function WalletListView() {
  const renderItem = ({ item }: { item: WalletRow }) => (
    <ListItem
      leading={
        <Avatar.Token
          media={
            <Typography.TextPrimary fontWeight='700'>{item.name.slice(0, 2).toUpperCase()}</Typography.TextPrimary>
          }
        />
      }
  ...
  return (
    <FlashList
      ...
      insetSafearea={false}
      ListHeaderComponent={() => (
        <YStack gap='$2' py='$4' pt='$8' px='$4' borderBottomWidth={1} borderBottomColor='$background2' mb='$4'>
          <Typography.TextSecondary color='$color10'>{WALLET_SELECTOR_TOTAL.title}</Typography.TextSecondary>
          <Typography.NumberHeading>{WALLET_SELECTOR_TOTAL.amount}</Typography.NumberHeading>
        </YStack>
      )}
    />
  );

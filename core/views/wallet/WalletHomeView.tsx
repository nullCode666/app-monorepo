...
export function WalletHomeView() {
  ...
  const defiActivities = useMemo(() => DEFI_ACTIVITY.filter(item => item.type === 'item').slice(0, 3) as ActivityItem[], []);
  ...
  return (
    ...
    >
      ...
      <Section title='DeFi' href='/list/defi'>
        <YStack px='$4'>
          {defiActivities.map(item => (
            <DefiActivityItem key={item.id} item={item} />
          ))}
        </YStack>
      </Section>

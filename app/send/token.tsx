...
export default function SendTokenScreen() {
  const { tokenId: tokenIdParam, tokenSymbol: tokenSymbolParam, tokenName: tokenNameParam, networkId: networkIdParam, networkLabel: networkLabelParam } =
  ...
  const tokenId = useMemo(() => toParam(tokenIdParam), [tokenIdParam]);
  ...
  }, [networkId, networkLabel, tokenId, tokenName, tokenSymbol]);
  ...
  return (
    <YStack flex={1} bg='$backgroundModal' pt={top} pb={bottom} px='$4' gap='$6' justifyContent='space-between'>
      ...
      <Button size='$4' onPress={handleProceed}>
        下一步
      </Button>
    </YStack>
  );

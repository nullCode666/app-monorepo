...
export default function SendNetworkScreen() {
  const { tokenId: tokenIdParam, tokenSymbol: tokenSymbolParam, tokenName: tokenNameParam } =
  ...
  useEffect(() => {
    if (!segments.length && tokenId && tokenSymbol) {
      router.replace({ pathname: '/send/token', params: { tokenId, tokenSymbol, tokenName } });
    }

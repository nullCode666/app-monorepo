...
export default function ListItemDemoScreen() {
  return (
    <DemoPage>
      <DemoSection title='基础布局' description='`ListItem` 利用 `bodyLeft` / `bodyRight` 插槽快速构建资产列表。'>
        <ListItem
          leading={
            <Avatar.Token media='https://uni.onekey-asset.com/server-service-indexer/btc--0/tokens/address-.png' />
          }
      ...
      <DemoSection title='带操作按钮' description='右侧可传入自定义 CTA 或状态信息。'>
        <ListItem
          ...
          bodyRightTop={
            <Typography.TextPrimary fontWeight='600' color='$primary'>
              View
      ...
      <DemoSection title='自定义间距与角标' description='通过角标媒体展示额外状态，结构保持左右两列。'>
        <ListItem
          leading={
            <Avatar.Token
      ...
      <DemoSection title='历史记录布局' description='复刻 `HistoryActivityItem` 中的两行布局，右侧包含数值与角标头像。'>
          ...
          bodyRightTop={
            <XStack alignItems='center' justifyContent='flex-end' gap='$2'>
              ...
              <Avatar.Token
                ...
                cornerMedia='https://i.meee.com.tw/WhPy9gB.png'
              />
            </XStack>
          }
          bodyRightBottom={<Typography.TextSecondary numberOfLines={1}>多种资产</Typography.TextSecondary>}
        />
